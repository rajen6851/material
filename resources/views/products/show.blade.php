<x-app-layout>
    ...
    <div class="bg-white border-b py-3 text-xs text-slate-500 font-semibold">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav class="flex gap-2 items-center">
                ...
                <span>&gt;</span>
                <a href="/products?category={{ $product->category->slug ?? 'tiles' }}"
                    class="hover:text-teal-600">{{ $product->category->name ?? 'Tiles' }}</a>
                <span>&gt;</span>
                <a href="/products?room={{ $product->room->slug ?? 'general' }}"
                    class="hover:text-teal-600">{{ $product->room->name ?? 'General' }} Tiles</a>
                <span>&gt;</span>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" x-data="{ 
        ...
        <div
            class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex justify-between items-center mb-1">
                        ...
                        <span
                            class="text-teal-700 font-bold text-sm tracking-wide uppercase">{{ $product->brand->name ?? 'Premium' }}</span>
                    </div>
                </div>

                <!-- Specs Grid -->
                <div
                    class="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6 border-y py-4 text-xs font-semibold text-slate-500">
                    <div>
                        <span class="block text-slate-400 font-medium">Size</span>
                        <span class="text-slate-800 font-bold">{{ $product->size }}
                            ({{ $product->size_inch ?: '24x48' }})</span>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <span class="text-slate-800 font-bold">{{ $product->coverage_per_box_sqft ?: '16' }}
                            sq.ft</span>
                    </div>
                    <div>
                    ...
                    <div>
                        @if($isProfessional && $product->wholesale_price)
                            <div class="flex items-baseline gap-2">
                                <span
                                    class="text-2xl font-black text-indigo-700">₹{{ number_format($product->wholesale_price, 2) }}
                                    / sq.ft</span>
                            </div>
                            <p class="text-[10px] text-slate-400 font-medium">Trade / wholesale price (excl. applicable
                                taxes)</p>
                        @else
                            <span class="text-2xl font-black text-slate-900">₹{{ number_format($product->price, 2) }} /
                                sq.ft</span>
                ...
                @if($isProfessional)
                    ...
                    @if($product->wholesale_price)
                        <div class="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 space-y-2 text-xs font-semibold">
                            <div class="flex items-center justify-between">
                                ...
                                <span
                                    class="text-slate-900 font-black text-sm">₹{{ number_format($product->wholesale_price, 2) }}
                                    / sq.ft</span>
                            </div>
                            <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center border border-slate-300 rounded-xl overflow-hidden h-10 bg-slate-50">
                            <button type="button" @click="quantity > 1 ? quantity-- : null"
                                class="px-3 text-slate-600 font-bold hover:bg-slate-100 transition">&minus;</button>
                            <input type="number" x-model="quantity"
                                class="w-12 border-none text-center bg-transparent focus:ring-0 font-bold text-slate-800 text-sm"
                                readonly>
                            <button type="button" @click="quantity++"
                                class="px-3 text-slate-600 font-bold hover:bg-slate-100 transition">&plus;</button>
                        </div>
                    </div>

                    <!-- Pro actions -->
                    <div class="space-y-3">
                        <a href="/quotation-requests/create?product_id={{ $product->id }}"
                            class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold h-12 rounded-xl transition text-sm flex items-center justify-center">
                            Request Bulk Quotation
                        </a>
                        <a href="{{ route('showroom.book') }}"
                            class="w-full bg-[#c09b5a] hover:bg-[#a58245] text-white font-bold h-12 rounded-xl transition text-sm flex items-center justify-center gap-2 shadow-sm">
                            <span>🏢</span>
                            <span>Book Trade Showroom Visit (B2B)</span>
                        </a>
                        <p class="text-[10px] text-slate-400 text-center">Bulk quotes are confirmed by our trade desk with
                            final pricing.</p>
                    </div>
                    ...
                    <div class="bg-slate-50 border p-5 rounded-2xl space-y-3 text-xs font-semibold text-slate-600">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span
                                class="text-slate-900 font-bold">₹{{ number_format($product->wholesale_price ?: $product->price, 2) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Quantity</span>
                ...
                @else
                    ...
                    <div class="flex gap-4">
                        <form action="/cart/add" method="POST" class="flex-1">
                            ...
                            <button type="submit"
                                class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold h-12 rounded-xl transition text-sm">
                                Add to Cart
                            </button>
                        </form>
                        <form action="/buy-now" method="POST" class="flex-1">
                            ...
                            <input type="hidden" name="quantity" :value="quantity">
                            <button type="submit"
                                class="w-full bg-teal-700 hover:bg-teal-800 text-white font-bold h-12 rounded-xl transition text-sm">
                                Buy Now
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="space-y-4">
                        <form action="/buy-now" method="POST">
                            ...
                            <input type="hidden" name="quantity" :value="quantity">
                            <button type="submit"
                                class="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold h-12 rounded-xl shadow-md transition flex items-center justify-center text-sm">
                                Proceed To Buy
                            </button>
                        </form>
                        ...
                        <div class="text-center">
                            <a href="/wishlist/add/{{ $product->id }}"
                                class="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-red-500 transition">
                        ...
                        <!-- Showroom Visit CTA for Homeowners -->
                        <div class="pt-1">
                            <a href="{{ route('showroom.book') }}"
                                class="w-full bg-[#faf7f2] hover:bg-[#f3ede3] text-[#171615] border border-[#ded7cd] font-bold h-11 rounded-xl transition text-xs flex items-center justify-center gap-2">
                                <span>📍</span>
                                <span>Book Showroom Visit (Touch &amp; Feel Samples)</span>
                            </a>
                        </div>
                    </div>
                @endif
        ...
        <div class="mt-12 bg-white border border-slate-200 rounded-3xl p-8" x-data="{ activeTab: 'desc' }">
            <div class="flex border-b text-sm font-bold text-slate-400 gap-6">
                <button @click="activeTab = 'desc'"
                    :class="activeTab === 'desc' ? 'border-b-2 border-teal-600 text-teal-700 pb-3' : 'pb-3'"
                    class="focus:outline-none">Description</button>
                <button @click="activeTab = 'info'"
                    :class="activeTab === 'info' ? 'border-b-2 border-teal-600 text-teal-700 pb-3' : 'pb-3'"
                    class="focus:outline-none">Additional Info</button>
                <button @click="activeTab = 'reviews'"
                    :class="activeTab === 'reviews' ? 'border-b-2 border-teal-600 text-teal-700 pb-3' : 'pb-3'"
                    class="focus:outline-none">Reviews (12)</button>
            </div>
            ...
            <div class="mt-6 text-sm text-slate-600 leading-relaxed">
                ...
                <div x-show="activeTab === 'info'" class="space-y-2">
                    ...
                    <p><strong>Estimated Delivery:</strong> {{ $product->delivery_time }} <span class="text-xs text-[#b58d56] font-bold">(Shipping Charges Applicable at actuals)</span></p>
                </div>
                <div x-show="activeTab === 'reviews'" class="space-y-4">
                    <div class="border-b pb-4">
                        <p class="font-bold text-slate-800">John Doe <span class="text-xs text-slate-400 font-normal">on
                                July 15, 2026</span></p>
                        <p class="text-xs text-amber-500 font-bold">★★★★★</p>
                        <p class="text-slate-600 text-xs mt-1">Excellent glossy finish! Totally matches my bathroom
                            color choice.</p>
                    </div>
                    <div class="border-b pb-4">
                        <p class="font-bold text-slate-800">Jane Smith <span
                                class="text-xs text-slate-400 font-normal">on July 18, 2026</span></p>
                        <p class="text-xs text-amber-500 font-bold">★★★★☆</p>
                        <p class="text-slate-600 text-xs mt-1">Quality is top notch, slight delay in delivery but very
                            satisfied with the packaging.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-12 bg-white border border-slate-200 rounded-3xl p-8 shadow-sm" x-data="{
            ...
            get tilesNeeded() { return this.tileAreaSqft > 0 ? Math.ceil(this.roomArea / this.tileAreaSqft) : 0; },
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 font-medium mt-1">Estimate how many tiles and boxes you need for your
                    room.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                <div class="space-y-4 md:col-span-1">
                    <div>
                        ...
                        <input type="number" min="0" step="0.1" x-model.number="length"
                            class="w-full border-slate-300 focus:border-teal-500 focus:ring-0 rounded-xl text-sm p-3">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Room Width (ft)</label>
                        <input type="number" min="0" step="0.1" x-model.number="width"
                            class="w-full border-slate-300 focus:border-teal-500 focus:ring-0 rounded-xl text-sm p-3">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Tile Area (sq.ft)</label>
                        <input type="number" min="0" step="0.01" x-model.number="tileAreaSqft"
                            class="w-full border-slate-300 focus:border-teal-500 focus:ring-0 rounded-xl text-sm p-3">
                    </div>
                    <button type="button" @click="estimated = true"
                        class="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 rounded-xl transition text-sm">Calculate</button>
                </div>

                <div class="md:col-span-2 bg-slate-50 border border-slate-200 rounded-2xl p-6" x-show="estimated"
                    x-cloak>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                        <div class="bg-white border rounded-xl p-4 shadow-sm">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Area</p>
                            <p class="text-xl font-black text-slate-900 mt-1"><span x-text="roomArea.toFixed(2)"></span>
                                <span class="text-xs font-semibold text-slate-400">sq.ft</span>
                            </p>
                        </div>
                        <div class="bg-white border rounded-xl p-4 shadow-sm">
                        ...
                        <div class="bg-white border rounded-xl p-4 shadow-sm">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Boxes
                                (~{{ $product->pieces_per_box ?: 2 }} / box)</p>
                            <p class="text-xl font-black text-teal-700 mt-1" x-text="boxesNeeded"></p>
                        </div>
                    </div>
                    <div class="mt-4 flex items-center justify-between bg-white border rounded-xl px-4 py-3"
                        x-show="costEstimate > 0" x-cloak>
                        <p class="text-xs font-bold text-slate-500">Estimated Material Cost (excl. shipping)</p>
                        <p class="text-lg font-black text-slate-900">₹<span
                                x-text="costEstimate.toLocaleString('en-IN', {maximumFractionDigits: 0})"></span></p>
                    </div>
                </div>
            </div>
        </div>
